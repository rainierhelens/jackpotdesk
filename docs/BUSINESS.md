...
4. Private digest is live for the operator only ([DIGEST.md](DIGEST.md)). No public signup. It stays the 10:00 a.m. PT letter (`scripts/draw-digest.ts`, `draw-digest.yml`).
5. Public daily recap on the live site, auto-published through GitHub at 5:00 a.m. Pacific every day, no per-recap approval. Same official data libraries as the private digest, a Pages deploy instead of an inbox. The digest stays 10:00 a.m. PT. Package: same-odds in the first sentence (reuse live site copy); last night’s official results vs last night’s Ladder #1–#3 as a scored replay; EV call using SKIP / ENTERTAIN ONLY / RARE PLUS; link to the live Ladder for tonight. Do not publish tonight’s #1 as a tip sheet (it crowds the ticket). Desk pick may be mentioned as the least-crowded board, not a forecast. Rank #1 is the strongest match to history, never “the winning pick.”
...
- **2026-08-20:** Official vs last night's Ladder is an append-only ledger (`src/data/ladderReplay.json`). One row per game and official date. First write wins. Each recap appends last night; optional `LADDER_REPLAY_BACKFILL` can fill a short older window. Descriptive archive only. Not a forecast scoreboard and not tonight's #1.
- **2026-08-20:** Public recap publish moves to 5:00 a.m. Pacific daily (`0 12 * * *` UTC while PDT). All 7 days. Private digest stays 10:00 a.m. PT on `draw-digest.yml`.
