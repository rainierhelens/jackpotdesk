...
The public sibling is [`/recap`](https://www.jackpotdesk.com/recap). `/recap` is a newest-first log of every morning. Dated copies live at `/recap/YYYY-MM-DD`. The recap and this letter both run at 5:00 a.m. Pacific daily (`0 12 * * *` UTC while PDT). Same official data libraries (`scripts/lib/deskLetter.ts`). The digest is tonight’s #1–#3 in one inbox. The recap opens each day with the desk strip (official vs last night #1), then the scored replay on the latest day and each permalink. It does not publish tonight’s #1. `deploy.yml` generates the page during the Pages build and writes it into the live site. No human merge per morning. If the letter and the site disagree, the site is source of truth.
