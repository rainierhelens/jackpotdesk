...
The public sibling is [`/recap`](https://www.jackpotdesk.com/recap). Latest recap is the default page. Dated copies live at `/recap/YYYY-MM-DD`. The recap and this letter both run at 5:00 a.m. Pacific daily (`0 12 * * *` UTC while PDT). Same official data libraries (`scripts/lib/deskLetter.ts`). The digest is tonight’s #1–#3 in one inbox. The recap is last night’s official results versus last night’s Ladder #1–#3, plus tonight’s EV call, plus a link to the live Ladder. It does not publish tonight’s #1. `deploy.yml` generates the page during the Pages build and writes it into the live site. Each recap also appends that official-vs-Ladder comparison to `src/data/ladderReplay.json` (one row per game and official date; first write wins). No human merge per morning. If the letter and the site disagree, the site is source of truth.
...
5. After that it runs daily at 5:00 a.m. Pacific, same clock as the public recap.
