...
- **Recap** (desk tab, [`/recap`](../public/recap/index.html)): public scored replay. Latest at `/recap`. Dated copies at `/recap/YYYY-MM-DD`. Last official results versus the Ladder that was live before those numbers. EV call SKIP / ENTERTAIN ONLY / RARE PLUS. Link to the live Ladder for tonight. Does not publish tonight’s #1. Each game has a tweet-length desk line (≤280) for copy/send. Auto-built in the Pages job at 5:00 a.m. Pacific daily. Same primary tab bar as Desk. See [DIGEST.md](DIGEST.md).
