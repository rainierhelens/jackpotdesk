...
- **Recap** (desk tab, [`/recap`](../public/recap/index.html)): night-desk blog of last night, newest first. Latest at `/recap`. Dated copies at `/recap/YYYY-MM-DD`. Each day opens with the tweet-length desk strip (official vs last night #1, N of 5 / N of 6, prize-if-played, ≤280). The latest day and each permalink then show the scored replay slips (official vs #1, rungs, EV). Older days on the index keep the strip and an “Open the slip” link. Same-odds stays in the page chrome / Lab footer, not the lede. Does not publish tonight’s #1. Auto-built in the Pages job at 5:00 a.m. Pacific daily from every `public/recap/YYYY-MM-DD.json`. The 5:00 a.m. stamp is last night, not tonight. Same primary tab bar as Desk. See [DIGEST.md](DIGEST.md).
- **Private digest**: daily operator email of Ladder #1–#3 plus the EV call, 5:00 a.m. Pacific. Not a public list. Same libraries and clock as the public recap. See [DIGEST.md](DIGEST.md).
